...
// FUNCIÓN PARA HACER FETCH CON TOKEN INTELIGENTE
...
const Productos = () => {
  ...
  const findClientIdByDni = async (dni) => {
    try {
      let response = await apiFetch(`${API_BASE}/clientes/dni/${dni}`);
      ...
      response = await apiFetch(`${API_BASE}/clientes`);
  ...
  const handleCreateClienteAndReserve = async () => {
    ...
    try {
      ...
      const response = await apiFetch(`${API_BASE}/reservas`);
